...
public class CartService implements ICartService {
	Map<Integer, Cart> carts = new HashMap<>();//lưu trữ cart
	...
	public void addToCart(int productId, int quantity) {
		...
		// cong don
		Cart cart = new Cart();
		Optional<Heo> h = this.repository.findById(productId);//truy vấn heo theo id
		cart.setPrice(h.get().getPrice());
		cart.setHeo(mapper.convertToDTO(h.get()));
		int updatedQuantity = currentQuantity + quantity;
		cart.setQuantity(updatedQuantity);
		carts.put(productId, cart);//cập nhật
...
//
	...
	public void buy() {
		...
		order.setUser(this.userMapper.convertToDto(user));
		 OrderDTO order1 =  this.orderService.save(order);//lưu order
		OrderDetailDTO orderDetail = new OrderDetailDTO();
		carts.forEach((t, u) -> { //duyệt qua từng phần tử
			Optional<Heo> heo = this.repository.findById(t);//truy vấn
		...
		this.repository.saveAllAndFlush(heos); //lưu tất cả và xóa tức thời
	...
	//entry trả về 1 gtri t/ư,có gt khoảng thời gian vòng lặp
	for (Map.Entry<Integer, Cart> entry : carts.entrySet()) { 
		Cart val = entry.getValue(); //lấy gtri của cart
		BigDecimal sl = new BigDecimal(val.getQuantity());
		
	   BigDecimal bigDecimal = new BigDecimal((val.getPrice().multiply(sl)) +"");//multiply:nhân lên
		System.out.println(bigDecimal);
		total =  total.add(bigDecimal);	//tổng của bigDecimal
	}
